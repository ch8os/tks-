package com.tks.resource.common.utils;
...
import com.tks.resource.common.entity.FileResult;
    ...
    public List<FileResult> uploadFile(MultipartFile[] files, String filePath) {
        ...
        try {
            ...
            if (!ftp.changeWorkingDirectory(baseProjectPath)) {
                throw new BusinessException("根目录不存在，需要创建");
            }
            // 切换到上传目录
...
//                    InputStream in = new FileInputStream(new File("D:\\code\\dpm_v2.0\\master\\dpm-db\\src\\main\\resources\\config\\config.properties"));
